...
void load_books(Book books[], int *book_count) {
    ...
    FILE *file = fdopen(fd, "r"); //this is to associate a fd with a FILE stream
    if (file == NULL) {
    ...
    *book_count = 0;
    while (fscanf(file, "%s %d %s %[^\n]", books[*book_count].isbn, &books[*book_count].available,
     books[*book_count].user_id, books[*book_count].title) != EOF) {
        (*book_count)++;
    }
...
void save_books(Book books[], int book_count) {
    ...
    // Set up the flock structure for an exclusive lock (write lock)
    struct flock lock;
    lock.l_type = F_WRLCK;
    lock.l_whence = SEEK_SET;
    lock.l_start = 0;
...
void issue_book(int client_socket, Book books[], int book_count, const char *userid) {
    ...
    pthread_mutex_lock(&file_mutex); //so we acquired the file_mutex
